...
async function checkBells() {
    const bells = await client.fetch('*[_type == "product" && sku match "GBBell*"]{title, sku}');
    console.log(`Found ${bells.length} Bell products in Sanity:`);
    console.log(JSON.stringify(bells, null, 2));
}

checkBells().catch(console.error);
