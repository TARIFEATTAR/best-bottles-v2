    ...
    <main className="w-full bg-white dark:bg-background-dark min-h-screen font-sans pb-32 lg:pb-20 overflow-x-hidden">
      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-6 py-4 md:py-6 border-b border-gray-100 dark:border-gray-800 mb-4 md:mb-8 sticky top-[72px] bg-white/90 dark:bg-background-dark/90 backdrop-blur z-30">
        <div className="text-xs font-bold tracking-widest uppercase text-gray-400 flex items-center space-x-2 md:space-x-3">
          ...
          <span>/</span>
          <span className="hover:text-primary cursor-pointer hidden md:inline">Roll-On Bottles</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-5">
            <div className="lg:sticky lg:top-28">
              ...
              <div className="bg-white dark:bg-[#1E1E1E] rounded-2xl md:rounded-3xl border border-gray-100 dark:border-gray-800 w-full h-[45vh] md:h-[55vh] max-h-[550px] flex items-center justify-center p-6 md:p-10 shadow-sm overflow-hidden group relative">
                
                {/* Dynamic Composite Image: Bottle + Roller + Cap */}
                <img 
                ...
                {/* Cap Badge - Shows which cap is displayed */}
                <div className="absolute top-4 right-4 md:top-6 md:right-6 bg-white/90 dark:bg-black/60 backdrop-blur px-3 py-1.5 rounded text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-300 border border-gray-200 dark:border-gray-700 flex items-center gap-1.5">
                  <div 
                ...
                {/* Roller Badge */}
                <div className="absolute bottom-4 left-4 md:bottom-6 md:left-6 bg-white/90 dark:bg-black/60 backdrop-blur px-3 py-1.5 rounded text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-300 border border-gray-200 dark:border-gray-700">
                ...
                {hasCompositeCapImage && (
                  <div className="absolute bottom-4 right-4 md:bottom-6 md:right-6 bg-green-500/90 backdrop-blur px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider text-white flex items-center gap-1">
                ...
                <div className="mt-4 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-700 p-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="w-20 h-20 bg-white dark:bg-white/10 rounded-lg border border-amber-200 dark:border-amber-600 flex items-center justify-center p-2">
                      ...
                      />
                      <div 
                        className="w-12 h-12 rounded-full border-2 border-amber-300 shadow-lg hidden"
                        style={{ backgroundColor: selectedCap.color }}
                      />
                    </div>
                    ...
                    <div className="flex-1">
                      <div className="text-[10px] font-bold uppercase tracking-widest text-amber-600 dark:text-amber-400 mb-1">
                        ⚠️ Selected Cap (Preview Only)
                      </div>
                      <div className="text-lg font-bold text-[#1e1e4b] dark:text-white">
                        {selectedCap.name}
                      </div>
                      <div className="text-xs text-gray-500 capitalize">
                        {selectedCap.finish} finish • Main image shows Black Dot cap
                      </div>
                    </div>
              ...
              {/* Success message for composite caps */}
              {hasCompositeCapImage && (
            ...
            {/* Mobile: Horizontal Selection Strip */}
            <div className="flex lg:hidden gap-3 overflow-x-auto py-4 px-1 no-scrollbar mt-2">
              ...
              {productData.baseBottles.map((bottle) => (
                <button 
                  key={bottle.id}
                  onClick={() => setSelection(s => ({ ...s, bottleId: bottle.id }))}
                  className={`w-16 h-16 shrink-0 rounded-lg border bg-white dark:bg-white/5 p-1 flex flex-col items-center justify-center ${
                    selection.bottleId === bottle.id 
                      ? 'border-[#C5A065] ring-1 ring-[#C5A065]' 
                      : 'border-gray-200 dark:border-gray-700'
                  }`}
                >
                  <img src={bottle.imageUrl} alt={bottle.name} className="w-8 h-8 object-contain" />
                  <span className="text-[8px] text-gray-500">{bottle.color}</span>
                </button>
              ))}
              
              <div className="w-px bg-gray-200 dark:bg-gray-700 mx-1 h-16 shrink-0"></div>
              
              {/* Cap Styles */}
              {productData.capOptions.slice(0, 5).map((cap) => {
                const capData = cap as typeof cap & { hasCompositeImage?: boolean };
                return (
                  <button 
                    key={cap.id}
                    onClick={() => setSelection(s => ({ ...s, capId: cap.id }))}
                    className={`w-16 h-16 shrink-0 rounded-lg border bg-white dark:bg-white/5 p-1 flex flex-col items-center justify-center gap-1 relative ${
                      selection.capId === cap.id 
                        ? 'border-[#C5A065] ring-1 ring-[#C5A065]' 
                        : 'border-gray-200 dark:border-gray-700'
                    }`}
                  >
                    ...
                    />
                    <div 
                      className="w-5 h-5 rounded-full border border-gray-300 hidden"
                      style={{ backgroundColor: cap.color }}
                    />
                    <span className="text-[8px] text-gray-500">{cap.name}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="lg:col-span-5">
            <div className="lg:sticky lg:top-28 flex flex-col space-y-5 md:space-y-6">
            ...
            <div>
              ...
              <h1 className="text-2xl md:text-3xl font-serif font-bold text-[#1e1e4b] dark:text-white mt-2 mb-2">
                {productName}
              </h1>
            ...
            <div className="bg-[#f0f0f0] dark:bg-[#252525] border border-gray-200 dark:border-gray-700 p-4 md:p-6 rounded-lg shadow-inner">
              ...
              <div className="flex gap-3">
                ...
                <button 
                  ...
                  className={`flex-1 md:flex-none px-4 py-4 md:py-3 border border-gray-300 dark:border-gray-600 rounded text-sm font-bold uppercase tracking-widest flex items-center justify-center gap-2 transition-colors ${
                    isSaved 
